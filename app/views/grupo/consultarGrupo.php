        ...
        <div class="content-wrapper" style="height: auto;">
            ...
            <div class="content pb-2">
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <input type="search" class="form-control rounded-0" id="searchbox" placeholder="Buscar" autocomplete="off">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-lg-12 table-responsive">
                            <table id="gridGrupo" class="table table-hover" role="grid" style="font-size: 12px; width: 100%; margin-top: 0px !important;">
                                <thead>
                                    <tr>
                                        <th>#Id</th>
                                        <th>Grupo</th>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            const option = {
                'success': (data) => {
                    ...
                    tableGrupo.state.save();

                    // Cerramos el tab editar
                    if ( data.accion == 'editar' ){
                        
                        // Acceder al iframe hijo por su nombre o ID
                        let iframeHijo = window.parent.document.getElementById('editar-grupo');
            ...
            tableGrupo = $('#gridGrupo').DataTable(
                UI.paramsDataTable( 
                    tableGrupoParam.length, 
                    tableGrupoParam.url, 
                    tableGrupoParam.data, 
                    tableGrupoParam.columns
                )
            );
            tableGrupo_filter = $('#gridGrupo').dataTable();
            $("#searchbox").keyup(function() {
                tableGrupo_filter.fnFilter(this.value);
            });

        });

        document.getElementById('btn_nuevo').addEventListener('click', function(e) {
            UI.create_tab('Nuevo grupo', RUTA_URL + 'grupo');
        });

        document.getElementById('btn_actualizar').addEventListener('click', function(e) {
            location.href = RUTA_URL + CONTROLADOR + '/consultarGrupo';
        });
        ...
        const opciones = {
            'editar': (id_grupo) => {
                ...
                setTimeout(function() {
                    UI.create_tab('Editar grupo', RUTA_URL + 'grupo/editar');
                    let iframeDestino = window.parent.document.getElementById("editar-grupo");
                    let url = RUTA_URL + CONTROLADOR + '/editar/' + id_grupo;
        ...
        document.getElementById('gridGrupo').addEventListener('click', function(e) {
            let id_grupo = e.target.getAttribute('data-id_grupo') ? e.target.getAttribute('data-id_grupo') : '';
            const opcion = opciones[e.target.name] ? opciones[e.target.name](id_grupo) : '';
        });
