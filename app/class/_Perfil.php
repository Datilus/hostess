...
class _Perfil extends Controller {
    ...
    public function __construct()
    {
        ...
        $this->modelPerfil = $this->model('PerfilModel');
	}

    /**
     *
     * @param number  $id_usuario  Número ID de Perfil
     */
    ...
    /**
     *
     * @param number  $id_perfil  Perfil de Perfil
     */
    ...
    public function guardar( $data_array )
    {
        ...
        $response = $this->modelPerfil->guardar( $this->data );

        $status_msg = ( $response['ERROR'] ) ? 'error' : 'success' ;

        throw new DataStatusResponse( $response['ERROR'], 0, 'Perfil guardado.', 201, ['status_msg' => $status_msg] );
    }

    public function obtener_usuario( $ban, $usuario )
    {
        return $this->modelPerfil->consultar(['ban'=> $ban, 'search' => $usuario, 'opciones' => ['tipo'=> "U"]]);
    }
    ...
    public function obtener_usuario_por_id( $id ) 
    {
		...
		$data = $this->modelPerfil->consultar( $data );
    ...
    public function borrar( $data_array ) 
    {
        ...
        // new ViewDataTest( $this->data );
        throw new DataStatusResponse( $response['ERROR'], 0, 'Perfil borrado.', 201, ['status_msg' => $status_msg] );
    ...
    public function data_table_list( $data_array = [] ) 
    {
		...
		$data = $this->modelPerfil->data_list($data);
